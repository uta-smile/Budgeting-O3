    ...
    def sample(
        ...
        **network_condition_kwargs,
        ...
        if deterministic and (
            ...
            raise ValueError("Deterministic PF-ODE sampling does not support steering")
        ...
        if initial_atom_coords is None:
            ...
            initial_atom_coords = initial_atom_coords.to(
                device=self.device, dtype=torch.float32
            )
            if initial_atom_coords.ndim != 3 or initial_atom_coords.shape[-1] != 3:
                raise ValueError(
                    "initial_atom_coords must have shape (batch, atoms, 3)"
                )
            if initial_atom_coords.shape[1:] != shape[1:]:
                raise ValueError(
                    "initial_atom_coords atom shape does not match atom_mask"
                )
            if initial_atom_coords.shape[0] == 1:
                initial_atom_coords = initial_atom_coords.expand(multiplicity, -1, -1)
            elif initial_atom_coords.shape[0] != multiplicity:
                raise ValueError(
                    "initial_atom_coords batch must be 1 or equal to multiplicity"
                )
            if not torch.isfinite(initial_atom_coords).all():
                raise ValueError("initial_atom_coords contains non-finite values")
            atom_coords = init_sigma * initial_atom_coords
        token_repr = None
        ...
        for step_idx, (sigma_tm, sigma_t, gamma) in enumerate(sigmas_and_gammas):
            if deterministic:
                ...
                if atom_coords_denoised is not None:
                    atom_coords_denoised = (
                        atom_coords_denoised
                        - atom_coords_denoised.mean(dim=-2, keepdims=True)
                    )
            else:
            ...
            t_hat = sigma_tm if deterministic else sigma_tm * (1 + gamma)
            steering_t = 1.0 - (step_idx / num_sampling_steps)
            noise_var = (
                0.0
                if deterministic
                else self.noise_scale**2 * (t_hat**2 - sigma_tm**2)
            )
            eps = (
                torch.zeros(shape, device=self.device)
                if deterministic
                else sqrt(noise_var) * torch.randn(shape, device=self.device)
            )
